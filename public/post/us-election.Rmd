...
Below(Table \@ref(tab:data)) is a preview

```{r data, echo=FALSE}
kable(head(cleaned_data[1:6]), caption = "Democracy Fund + UCLA Nationscape Survey data preview") 
```

```{r}
kable(head(cleaned_data[7:13])) 
```
...
Also, Figure \@ref(fig:vote2016) not only suggests that Trump has a large number of repeated voters, but it also shows that Biden has gained the majority of voters who voted Hilary Clinton in 2016, as well as people who did not vote.(Figure \@ref(fig:vote2016)) This finding is significant as it shows more people have decided to vote. 

Figure \@ref(fig:gender) to Figure \@ref(fig:income) visualizes the distribution of some explanatory variables for our model. Here are some interesting findings:
...
raw_data <- read_dta("usa_00006.dta")
# Add the labels
raw_data <- labelled::to_factor(raw_data)
names(raw_data)
...
Below is a preview of the finalized cleaned dataset(Table \@ref(tab:acspreview)) with counts for each subgroup. However, it should be notified that the count for post stratification can be relatively small to the overall size of the dataset since we have state as our predictor, which can have 50 possible values.

```{r acspreview, echo=FALSE}
knitr::kable(head(cleaned_data_strat_count), caption = "ACS Data Preview")
```


Moreover, Figure \@ref(fig:varpie2) shows the distribution of our predictor variables
...
There are a few downfalls with this model. Firstly, the independent variables in our model are categorical rather than continuous. This means that our regression line will not be following a trend as the independent variables are discrete and will not follow a linear trend. Additionally, if we take a look at the normal-QQ plot(figure \@ref(fig:qq)), we can see that our data shows a more S-shape rather than a linear relationship. 

```{r qq, include=FALSE, fig.cap = "OLS Model Regression"}
...
To validate this model, we can perform a series of checks. First we can check the Normal QQ plot in figure \@ref(fig:newqq) to check to see if the fit of our model is better than the OLS Linear Model:

```{r newqq, echo = FALSE, fig.cap = "GLM Model"}
...
We can see that the anova checks resulted in a larger deviation after dropping the race variable. Additionally, the AIC value increased thus confirming that this is a significant variable
...
```{r statepred, fig.cap="Popular vote perdiction by State", echo =FALSE, message=FALSE}
